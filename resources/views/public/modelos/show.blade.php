<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Verifica se deve mostrar a view simplificada --}}
            @php
            ...
            @if($mostrarViewSimplificada)
                {{-- View Simplificada: Acesso Direto ao Manual HTML --}}
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <a href="{{ route('manuais.html.view', $manualUnico) }}" target="_blank"
            ...
            @else
                {{-- View Padrão: Busca e Card de Manuais --}}

                {{-- Seção de Busca nos Manuais do Modelo --}}
                <div class="mb-8 pb-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <form action="{{ route('modelos.show', $modelo) }}" method="GET" class="flex items-center space-x-3 mb-6 max-w-xl">
                        {{-- ... input, botão, link limpar ... --}}
                         <input type="search" name="q_modelo" placeholder="Digite código ou termo para buscar nos manuais..." value="{{ $queryBuscaManual ?? '' }}" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:focus:border-indigo-600 dark:focus:ring-indigo-500/50">
                                            ...
                                            @if ($ref->manual) {{-- Segurança extra --}}
                                                <li class="py-3">
                                                    <div class="flex justify-between items-center">
                                                        {{-- ... conteúdo do resultado da busca ... --}}
                                                         <div>
                                                            <span class="font-semibold text-indigo-600 dark:text-indigo-400">{{ $ref->codigo_encontrado }}</span>
                                                            encontrado em
                                                            {{-- Link condicional --}}
                                                            @if($ref->manual->tipo === 'html')
                                                                <a href="{{ route('manuais.html.view', $ref->manual) }}" target="_blank" class="font-semibold underline hover:text-indigo-500">{{ $ref->manual->nome }} (HTML)</a>
                                                            @else {{-- Assume PDF --}}
                                                                <a href="{{ route('manuais.view', ['manual' => $ref->manual->slug, 'page' => $ref->numero_pagina]) }}" target="_blank" class="font-semibold underline hover:text-indigo-500">{{ $ref->manual->nome }}</a> (página {{ $ref->numero_pagina }})
                                                            @endif
                                                        </div>
                                                        {{-- Botão condicional --}}
                                                        @if($ref->manual->tipo === 'html')
                                                            <a href="{{ route('manuais.html.view', $ref->manual) }}" target="_blank" class="text-sm text-purple-600 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-300">Abrir HTML &rarr;</a>
                                                        @else {{-- Assume PDF --}}
                                                            <a href="{{ route('manuais.view', ['manual' => $ref->manual->slug, 'page' => $ref->numero_pagina]) }}" target="_blank" class="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">Abrir PDF &rarr;</a>
                                                        @endif
                                                    </div>
                                                </li>
                                            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 text-gray-900 dark:text-gray-100">
                            {{-- ... conteúdo do card de manuais ... --}}
                             <h3 class="text-lg font-medium mb-2">Manuais</h3>
                            @if($modelo->manuais_count > 0)
                                <p class="text-sm text-gray-600 mb-4">{{ $modelo->manuais_count }} {{ Str::plural('manual', $modelo->manuais_count) }} disponível{{ Str::plural('s', $modelo->manuais_count) }} para este modelo.</p>
            ...
            @endif {{-- Fim da condição if/else principal --}}
